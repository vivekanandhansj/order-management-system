...
function Home() {
  ...
  async function fetchAll() {
    try {
      let studentsData = await axios.get("https://oms-server.onrender.com/api/order/orderList", {
        headers: {
  ...
  let handleDelete = async (id) => {
    try {
      ...
      if (ask) {
        await axios.delete(`https://oms-server.onrender.com/api/order/deleteOrder/${id}`, {
          headers: {
            Authorization: window.localStorage.getItem("myapptoken"),
          },
        });
